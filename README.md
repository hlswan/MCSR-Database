...
A SQLite database and Python import pipeline for Minecraft Java Edition 1.16.1 RSG Any%. Trying to get all sub10s but currently only have 629 or something. **Every sub8 should be on the sheet though**(as of 14 Apr, '26).

The leaderboard is trying to track every sub-10 minute speedrun but I don't have them all(yet). Stop being so cracked people. I estimate I probably have 30% of all sub10 runs. This database does include every sub8. The database is generated from a Google Sheets export maintained by me(hiya!) and (hopefully) will eventually power a web frontend. Google sheet link: https://docs.google.com/spreadsheets/d/1zgmOYJBULyHLqs9lGB6-cO4QhRoJdgOpd9WpUWpILYo/edit?usp=sharing
